...
import { IIssueMainService, IIssueMainService2, IssueReporterData, IssueReporterWindowConfiguration, ProcessExplorerData, ProcessExplorerWindowConfiguration } from 'vs/platform/issue/common/issue';
...
export class IssueFormService implements IIssueMainService {
	...
	private configuration: IssueReporterWindowConfiguration | undefined = undefined;
	...
	constructor(
		...
		@IIssueMainService2 private readonly issueMainService: IIssueMainService2,
		...
		// @INativeHostMainService private readonly nativeHostService: INativeHostService,
	...
	private registerListeners(): void {
		...
		// 		const remoteDiagnostics = await this.diagnosticsService.getDiagnostics({ includeProcesses: true });
	...
	async openReporter(data: IssueReporterData): Promise<void> {
		if (!this.issueReporterWindow) {
			...
			// Store into config object
			this.configuration = {
				appRoot: this.nativeEnvironmentService.appRoot,
				windowId: 0,
				userEnv: {},
				data,
				disableExtensions: !!this.nativeEnvironmentService.disableExtensions,
				os: {
					type: '',
					arch: '',
					release: '',
	...
	async openAuxIssueReporter(data: IssueReporterData): Promise<void> {
		...
		// Auxiliary Window
		const auxiliaryWindow = disposables.add(await this.auxiliaryWindowService.open({ bounds: { width: 700, height: 750 } }));
		...
		if (auxiliaryWindow) {
			...
			if (this.configuration) {
				const issueReporter = this.instantiationService.createInstance(IssueReporter2, this.configuration, auxiliaryWindow.window);
	...
	async openProcessExplorer(data: ProcessExplorerData): Promise<void> {
		...
		// 			FileAccess.asBrowserUri(`vs/code/electron-sandbox/processExplorer/processExplorer${this.nativeEnvironmentService.isBuilt ? '' : '-dev'}.html`).toString(true)
		// 		);
	...
	async stopTracing(): Promise<void> {
		if (!this.nativeEnvironmentService.args.trace) {
			return; // requires tracing to be on
		}
		// const path = await contentTracing.stopRecording(`${randomPath(this.nativeEnvironmentService.userHome.fsPath, this.productService.applicationName)}.trace.txt`);
		...
		// }, Window.getFocusedWindow() ?? undefined);

		// Show item in explorer
		// this.nativeHostService.showItemInFolder(path);
	}

	async getSystemStatus(): Promise<string> {
		// const [info, remoteData] = await Promise.all([this.diagnosticsService.getDiagnostics()]);
		// 	// .getMainDiagnostics(), this.diagnosticsMainService.getRemoteDiagnostics({ includeProcesses: false, includeWorkspaceMetadata: false })]);
	...
	async $getSystemInfo(): Promise<SystemInfo> {
		...
		// return msg;
		// this.issueMainService.$getPerformanceInfo();
...
// 			y: position.y,
// 			title: options.title,
// 			backgroundColor: options.backgroundColor || IssueFormService.DEFAULT_BACKGROUND_COLOR,
// 			webPreferences: {
